...
import operatorRegMigrateRejectedValidationSchema from '../../../validations/operatorRegMigrateRejectedValidationSchema';
...
export default function OperatorRegistrationMigrateAdminRejected(props) {
    ...
    const notify = () => toast.info("Operator Approved!");
    ...
    const formik = useFormik({
        ...
        initialValues: {
            ...
            isMigrate: false,
            rejectedReason: operatorData?.rejectedReason ? operatorData?.rejectedReason : '',
            rejectedDate: operatorData?.rejectedDate ? operatorData?.rejectedDate : '',
            address: {
        ...
        validationSchema: operatorRegMigrateRejectedValidationSchema,
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setisDisable(true)
            const { data: { data } } = await authAxios.put(`operator/update/status/${operatorId}?status=APPROVED`)
            notify()
            console.log(data)
            setTimeout(() => {
                setisDisable(false)
            ...
            <Paper sx={{ flexGrow: 1 }} className="OperatorRegNew" elevation={10} style={{ padding: '1.25rem' }}>
                ...
                <form onSubmit={formik.handleSubmit}>
                        ...
                        <input id="fileUrl" className="phoneNumberInput" variant="standard"
                            ...
                            onChange={(event) => {
                                ...
                                reader.onload = () => {
                                    ...
                                    // 1 means loading
                                    // 2 means done
                                    if (reader.readyaddress?.State === 2) {
                    ...
                    <input
                        ...
                        onChange={formik.handleChange}
                        placeholder="ie. XXXXX" />


                    <label className="loginLable" htmlFor="rejectedReason">Rejected Reason</label>

                    {formik.errors.rejectedReason && formik.touched.rejectedReason && <Alert severity="warning">{formik.errors.rejectedReason}</Alert>}
                    ...
                    <label className="loginLable" htmlFor="rejectedDate">Rejected Date</label>

                    {formik.errors.rejectedDate && formik.touched.rejectedDate && <Alert severity="warning">{formik.errors.rejectedDate}</Alert>}
                    ...
                    <input
                        ...
                        type={"text"}
                        name="rejectedDate"
                        value={formik.values.rejectedDate}
                        onChange={formik.handleChange}
                        placeholder="ie. XX-XX-XXXX" />
